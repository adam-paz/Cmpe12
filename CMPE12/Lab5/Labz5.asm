...
Bloop	AND R3, R3, #0
	ADD R3, R3, #1	; Outer loop in order to move the pointer and compare with the pointer
	AND R4, R4, #0
	ADD R4, R4, R5

Inloop	ADD R3, R3, R3	;Inner loop in order for pointer to move right instead of left
	ADD R4, R4, #-1
	BRp Inloop
	...
	LEA R0, one	;output a 1
	PUTS
	BRnzp count

ZERO	LEA R0, zero	;output a 0
	PUTS
...
another	LEA R0, DAWG
	PUTS
	ADD R7,R7,R7
	BRnzp again	;loop back to beggining

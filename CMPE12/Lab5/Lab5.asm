...
GETZ	ADD R1, R1, #-10	;Check for X
	...
	AND R3, R3, #0
	ADD R3, R3, R2		;Setup for multiplication
	AND R6, R6, #0
	ADD R6, R6, #9
...
GETFULL	ADD R2, R2, R3		;Multiplies R2 by 10
	...
	BRnzp GETNX		; goes back to get next digit
...
CONT1	AND R3, R3, #0		;Setup for bottom loop	
	AND R5, R5, #0		;R5 = 15 and R3 = 1 to be added to itself
	ADD R3, R3, #1		
	ADD R5, R5, #15
	AND R6, R6, #0
	
Bloop	ADD R5, R5, #0		;check if count is < 0
	BRn AGAIN
